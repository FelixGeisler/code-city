...
let faceShadingPass=false;
{
  ...
  const maximumUploadStart=state.uploads.length;const maximumDrawStart=state.draws;
  const maximumResult=stageCommit(presenter,holder.host,4000,maximumCity);
  presentation.maximum={result:maximumResult,groups:maximumCity.presentation.plates.length,uploads:state.uploads.slice(maximumUploadStart).map((bytes)=>bytes.length),draws:state.draws-maximumDrawStart,passKinds:state.uniforms.filter(({name})=>name==="u_passKind").slice(-2).map(({value})=>value)};
