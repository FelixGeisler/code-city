...
function presentationPlatform(state,compileFailure=false){
  return {createCanvas(){
    ...
    Object.defineProperty(canvas,"getContext",{value(kind,attributes){
      ...
      state.cueIdentities??=[];
      const programKinds=new Map();const vaoKinds=new Map();
      ...
      return new Proxy(actual,{get(target,property){
        ...
        if(property==="drawElementsInstanced")return (...args)=>{const program=programKinds.get(currentProgram);const vao=vaoKinds.get(currentVao);if(program!==vao)throw new Error("Bound program/VAO identity differs");if(args[0]===0x0001){if(program==="hover"){state.hoverDraws++;state.operations.push("hover");}else if(program==="selection"){state.outlineDraws++;state.operations.push("outline");}else throw new Error("Line draw used an unknown cue identity");state.cueIdentities.push({kind:program,program,vao});}else{if(program!=="city")throw new Error("Fill draw used a cue identity");state.draws++;state.operations.push("city");}return target.drawElementsInstanced(...args);};
...
let faceShadingPass=false;
{
  ...
  const state={canvases:[],draws:0,outlineDraws:0,hoverDraws:0,actualContexts:0,observerCallbacks:[],lossCallbacks:[],listenerAdds:[],listenerRemoves:[],uploads:[],subUploads:[],shaderSources:[],operations:[],polygonOffsetEnables:0,contextLost:false,deletes:{deleteShader:0,deleteProgram:0,deleteBuffer:0,deleteVertexArray:0}};
  const failures=[];
  ...
  const selectionIdentityStart=state.cueIdentities.length;
  ...
  const selection={cityDraws:state.draws-beforeSelectionCity,outlineDraws:state.outlineDraws-beforeSelectionOutline,operations:state.operations.slice(operationStart),identities:state.cueIdentities.slice(selectionIdentityStart)};
  const hoverOperationStart=state.operations.length;
  const hoverIdentityStart=state.cueIdentities.length;
  const beforeHoverCity=state.draws;const beforeHoverOutline=state.outlineDraws;const beforeHover=state.hoverDraws;
  if(presenter.setVisualState(2,0,0).kind!=="applied")throw new Error("Actual WebGL2 hover cue failed");
  const hoverEvidence={cityDraws:state.draws-beforeHoverCity,outlineDraws:state.outlineDraws-beforeHoverOutline,hoverDraws:state.hoverDraws-beforeHover,operations:state.operations.slice(hoverOperationStart),identities:state.cueIdentities.slice(hoverIdentityStart),updateBytes:state.subUploads[1],fullBoxIndices:state.uploads[16],fixedSlate:state.shaderSources.some((source)=>source.includes("o_color = vec4(148.0 / 255.0, 163.0 / 255.0, 184.0 / 255.0, 1.0);"))};
  if(presenter.setVisualState(2,null,0).kind!=="applied")throw new Error("Actual WebGL2 hover clear failed");
  hoverPass=JSON.stringify({cityDraws:hoverEvidence.cityDraws,outlineDraws:hoverEvidence.outlineDraws,hoverDraws:hoverEvidence.hoverDraws,operations:hoverEvidence.operations,identities:hoverEvidence.identities})===JSON.stringify({cityDraws:1,outlineDraws:1,hoverDraws:1,operations:["depth:on","city","depth:off","outline","hover","depth:on"],identities:[{kind:"selection",program:"selection",vao:"selection"},{kind:"hover",program:"hover",vao:"hover"}]})&&hoverEvidence.updateBytes.length===24&&JSON.stringify(hoverEvidence.updateBytes)===JSON.stringify(state.uploads[11].slice(0,24))&&JSON.stringify(hoverEvidence.fullBoxIndices)===JSON.stringify([0,1,1,2,2,3,3,0,4,5,5,6,6,7,7,4,0,4,1,5,2,6,3,7])&&hoverEvidence.fixedSlate;
  faceShadingPass=state.shaderSources.some((source)=>source.includes("base + 0.12 * (vec3(1.0) - base)")&&source.includes("0.82 * base")&&source.includes("0.62 * base"));
  ...
  presentation.outline={allocationUploads:state.uploads.map((bytes)=>bytes.length),updateBytes:state.subUploads[0],exactReplica:JSON.stringify(state.subUploads[0])===JSON.stringify(state.uploads[11].slice(0,24)),selection,camera,resizeOutlineDraws,reset,clear,immutableUploads:JSON.stringify(state.uploads)===immutableUploads,shaderFixedOffWhite:state.shaderSources.some((source)=>source.includes("o_color = vec4(248.0 / 255.0, 250.0 / 255.0, 252.0 / 255.0, 1.0);")),polygonOffsetEnables:state.polygonOffsetEnables,cleanup:null};
...
}
{
  ...
  if(event.defaultPrevented||presentation.lossDraws!==0||state.lossCallbacks.length!==1||typeof retainedLoss!=="function"||failures.length!==1||JSON.stringify(failures[0])!==JSON.stringify([3,"Presentation failed","M1-PRES-1"])||JSON.stringify(presentation.lossOrdering)!==JSON.stringify({semanticPresentAtNotification:true,hostChildrenAtNotification:2,cleanupAtNotification:{deleteShader:6,deleteProgram:0,deleteBuffer:0,deleteVertexArray:0},semanticPresentAfterControllerClear:false})||terminalLossCleanup!==JSON.stringify({deleteShader:6,deleteProgram:0,deleteBuffer:0,deleteVertexArray:0})||holder.host.firstChild!==null)throw new Error("Actual WebGL2 context-loss evidence failed");
