  ...
  return {createCanvas(){
    ...
    Object.defineProperty(canvas,"getContext",{value(kind,attributes){
      ...
      return new Proxy(actual,{get(target,property){
        ...
        if(property==="drawElementsInstanced")return (...args)=>{if(args[0]===0x0001&&args[1]===8){state.hoverDraws++;state.operations.push("hover");}else if(args[0]===0x0001){state.outlineDraws++;state.operations.push("outline");}else{state.draws++;state.operations.push("city");}return target.drawElementsInstanced(...args);};
...
let faceShadingPass=false;
{
  ...
  const hoverEvidence={cityDraws:state.draws-beforeHoverCity,outlineDraws:state.outlineDraws-beforeHoverOutline,hoverDraws:state.hoverDraws-beforeHover,operations:state.operations.slice(hoverOperationStart),updateBytes:state.subUploads[1],topIndices:state.uploads[16],fixedMagenta:state.shaderSources.some((source)=>source.includes("o_color = vec4(1.0, 0.0, 1.0, 1.0);"))};
  if(presenter.setVisualState(2,null,0).kind!=="applied")throw new Error("Actual WebGL2 hover clear failed");
  hoverPass=JSON.stringify({cityDraws:hoverEvidence.cityDraws,outlineDraws:hoverEvidence.outlineDraws,hoverDraws:hoverEvidence.hoverDraws,operations:hoverEvidence.operations})===JSON.stringify({cityDraws:1,outlineDraws:1,hoverDraws:1,operations:["depth:on","city","depth:off","outline","hover","depth:on"]})&&hoverEvidence.updateBytes.length===24&&JSON.stringify(hoverEvidence.updateBytes)===JSON.stringify(state.uploads[11].slice(0,24))&&JSON.stringify(hoverEvidence.topIndices)===JSON.stringify([4,5,5,6,6,7,7,4])&&hoverEvidence.fixedMagenta;
