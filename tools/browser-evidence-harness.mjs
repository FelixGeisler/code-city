...
}
{
  ...
  if(event.defaultPrevented||presentation.lossDraws!==0||failures.length!==1||presentation.lossOrdering.cleanupAtNotification.deleteShader!==2||presentation.lossOrdering.cleanupAtNotification.deleteProgram!==0||holder.host.firstChild!==null)throw new Error("Actual WebGL2 context-loss evidence failed");
...
}
{
  ...
  const retainedCompileLoss=state.lossCallbacks[0];const terminalCompileDraws=state.draws;const terminalCompileCleanup=JSON.stringify(state.deletes);retainedCompileLoss(new Event("webglcontextlost",{cancelable:true}));state.canvases[0].dispatchEvent(new Event("webglcontextlost",{cancelable:true}));
...
presentation.pass=focusPass&&faceShadingPass&&presentation.webgl2Available&&presentation.actualContexts===4&&presentation.initialDraws===1&&presentation.repeatDraws===1&&presentation.resizeDraws===1&&JSON.stringify(presentation.focus.allocationUploads)===JSON.stringify([384,36,28,384,36,28])&&presentation.focus.immutableUploads&&presentation.focus.subUploads===0&&presentation.focus.shaderFocus&&presentation.focus.faceShading&&presentation.focus.polygonOffsetEnables===0&&JSON.stringify(presentation.focus.cleanup)===JSON.stringify({deleteShader:4,deleteProgram:2,deleteBuffer:6,deleteVertexArray:2})&&JSON.stringify(presentation.accessibility)===JSON.stringify({tabIndex:0,label:"Interactive code city",description:"city-navigation-instructions",listenerAdds:["webglcontextlost","keydown","wheel","pointerdown","pointermove","pointerup","pointercancel","pointerleave","lostpointercapture","contextmenu","blur","visibilitychange","pagehide"],resetText:"Reset view"})&&presentation.lossDefaultPrevented===false&&presentation.lossDraws===0&&presentation.lossFailures.length===1&&presentation.lossOrdering.semanticPresentAtNotification&&presentation.lossOrdering.hostChildrenAtNotification===2&&presentation.lossCleanup.deleteShader===2&&presentation.lossCleanup.deleteProgram===0&&presentation.lossTerminalState.failures===1&&presentation.lossTerminalState.drawsAfterTerminal===0&&presentation.lossTerminalState.hostChildren===0&&presentation.lossTerminalState.cleanupUnchanged&&presentation.compileFailureResult.kind==="failure"&&presentation.compileFailureDraws===0&&presentation.compileFailures.length===0&&presentation.compileCleanup.deleteShader===1&&presentation.compileFailureTerminalState.failures===0&&presentation.compileFailureTerminalState.drawsAfterTerminal===0&&presentation.compileFailureTerminalState.hostChildren===0&&presentation.compileFailureTerminalState.cleanupUnchanged;
