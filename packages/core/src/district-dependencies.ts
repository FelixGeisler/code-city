...
export interface ExternalDistrictDependencyEndpoint {
  ...
  /**
   * The normalized target without a physical projection, otherwise the
   * privacy-neutral display label of the selected physical node.
   */
  readonly target: string;
  /**
   * Stable physical-node identity when an external projection was supplied.
   */
...
/**
 * Minimal physical external-node projection accepted by the shared selector.
 * Both ExternalDependencySelection and ExternalDependencyLayout satisfy it.
 */
export type DistrictDependencyExternalProjection = Pick<
  ExternalDependencySelection,
  "assignments" | "nodes"
>;
...
/**
 * Coalesces logical external targets by their selected physical node before
 * ranking and capping. In particular, overflow members become one
 * source-district-to-Others bundle without retaining member names in its
 * public target identity.
 */
function projectExternalBundles(
  bundles: readonly IndexedBundle[],
  projection: DistrictDependencyExternalProjection | undefined,
): readonly IndexedBundle[] {
  if (projection === undefined) {
    return bundles;
  }

  const nodesById = indexProjectionNodes(projection.nodes);
  const projectedById = new Map<
    string,
    {
      readonly endpoint: Pick<
        IndexedBundle,
        "id" | "sourceDistrictId" | "target"
      >;
      readonly edges: IndexedDependencyEdge[];
    }
  >();
  ...
  for (const bundle of bundles) {
    ...
    const endpoint =
      target.kind === "district"
        ? {
            id: bundle.id,
            sourceDistrictId: bundle.sourceDistrictId,
            target,
          }
        : projectExternalBundleEndpoint(
            bundle.sourceDistrictId,
            target,
            projection.assignments,
            nodesById,
          );
    const aggregate = projectedById.get(endpoint.id);
    if (aggregate === undefined) {
      projectedById.set(endpoint.id, {
        endpoint,
        edges: [...bundle.edges],
      });
    } else {
      aggregate.edges.push(...bundle.edges);
    }
  }

  return Object.freeze(
    [...projectedById.values()]
      .map(({ endpoint, edges }): IndexedBundle =>
        Object.freeze({
          ...endpoint,
          edges: Object.freeze([...edges].sort(compareIndexedEdges)),
        }),
      )
      .sort(compareIndexedBundles),
  );
...
function indexProjectionNodes(
  nodes: readonly ExternalDependencySelectionNode[],
): ReadonlyMap<string, ExternalDependencySelectionNode> {
  const nodesById = new Map<string, ExternalDependencySelectionNode>();
  for (const node of nodes) {
    if (nodesById.has(node.id)) {
      throw new TypeError(
        `External dependency projection contains duplicate node "${node.id}".`,
      );
    }
...
function projectExternalBundleEndpoint(
  sourceDistrictId: string,
  target: Extract<IndexedBundle["target"], { kind: "external" }>,
  assignments: ReadonlyMap<string, string>,
  nodesById: ReadonlyMap<string, ExternalDependencySelectionNode>,
): Pick<IndexedBundle, "id" | "sourceDistrictId" | "target"> {
  const nodeId = assignments.get(target.target);
  if (nodeId === undefined) {
    throw new RangeError(
      `External dependency projection does not assign target "${target.target}".`,
    );
  }
  const node = nodesById.get(nodeId);
  if (node === undefined) {
    throw new RangeError(
      `External dependency projection references unknown node "${nodeId}".`,
    );
  }
  return Object.freeze({
    id: bundleId(
...
function summarizeBundle(
  ...
  const contributors = Object.freeze(
    edges
      .map((edge) => toContributor(edge, bundle.target))
    ...
    target:
      bundle.target.kind === "district"
        ...
        : Object.freeze({
            ...
            ...(bundle.target.nodeId === undefined
              ? {}
              : { nodeId: bundle.target.nodeId }),
          }),
...
function toContributor(
  edge: IndexedDependencyEdge,
  bundleTarget: IndexedBundle["target"],
  ...
  const target =
    edge.target.kind === "district"
      ...
      : bundleTarget.kind === "external" &&
          bundleTarget.nodeId !== undefined
        ? {
            label: bundleTarget.target,
            path: bundleTarget.target,
          }
      : {
...
function indexedBundleTargetIdentity(bundle: IndexedBundle): string {
  return bundle.target.kind === "district"
    ...
    : `external:${bundle.target.nodeId ?? bundle.target.target}`;
}
